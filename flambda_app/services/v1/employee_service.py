...
from flambda_app.exceptions import (
    DatabaseException,
    ServiceException,
    ValidationException,
)
...
from flambda_app.connections.turnstile_connection import TurnstileConnection
...
class EmployeeService:
    ...
    def __init__(
        ...
        self.mysql_connector = (
            mysql_connector if mysql_connector is not None else MySQLConnector()
        )
        # mysql repository
        self.employee_repository = (
            employee_repository
            if employee_repository is not None
            else EmployeeRepository(
                mysql_connection=self.mysql_connector.get_connection()
            )
        )

        self.turnstile_connection = TurnstileConnection()
        ...
        if self.REDIS_ENABLED:
            ...
            self.redis_connector = (
                redis_connector if redis_connector is not None else RedisConnector()
            )
            # redis repository
    ...
    def list(self, request: dict):
        self.logger.info(
            "method: {} - request: {}".format(get_function_name(), request)
        )
        ...
        try:
            ...
            if data:
                ...
                for item in data:
                    vo_data.append(
                        EmployeeVO(item, default_values=False).to_api_response()
                    )
                data = vo_data
    ...
    def get(self, request: dict, id):
        ...
        self.logger.info("method: {} - uuid: {}".format(get_function_name(), id))
    ...
    def create(self, request: dict):
        ...
        try:
            ...
            if created:
                ...
                self.turnstile_connection.create_employee_turnstile(
                    employee_turnstile_input_vo
                )
                # convert to vo and prepare for api response
    ...
    def update(self, request: dict, id):
        ...
        original_employee = self.employee_repository.get(
            id, key=self.employee_repository.PK
        )
        ...
        try:
            ...
            updated = self.employee_repository.update(
                employee_vo, id, key=self.employee_repository.PK
            )
            ...
            if updated:
                ...
                self.turnstile_connection.patch_employee_turnstile(
                    employee_turnstile_input_vo
                )
                # convert to vo and prepare for api response
                data = employee_vo.to_api_response()
    ...
    def delete(self, request: dict, id):
        ...
        try:
            updated = self.employee_repository.soft_delete(
                value=id, key=self.employee_repository.PK
            )

            if updated:
                result = True
                self.turnstile_connection.delete_employee_turnstile(
                    original_employee.to_dict().get("uuid")
                )
            else:
                # set exception if it happens
