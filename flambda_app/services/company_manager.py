...
class CompanyManager:
    def __init__(self, logger=None, config=None, company_service=None):
        ...
        # service
        self.company_service = company_service if company_service is not None else CompanyService(self.logger)
    ...
    def get(self, request: dict, uuid):
        data = self.company_service.get(request, uuid)
    ...
    def soft_update(self, request: dict, uuid):
        data = self.company_service.soft_update(request, uuid)
        if (data is None) and self.company_service.exception:
            self.exception = self.company_service.exception
            raise self.exception
        return data
